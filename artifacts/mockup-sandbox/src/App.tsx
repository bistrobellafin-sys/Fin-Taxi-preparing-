...
function App() {
  ...
  const [selectedModule, setSelectedModule] = useState("driver-path");
  ...
  return (
    <div className={`app-shell ${darkMode ? "dark-app" : ""}`} dir="rtl">
      ...
      <main className="main-content">
        ...
        {activeNav === "home" && <Dashboard onStartQuiz={startQuiz} onNavigate={navigate} onOpenModule={(id) => { setSelectedModule(legacyModuleMap[id] ?? id); setActiveNav("path"); }} />}
...
function Dashboard({ onStartQuiz, onNavigate, onOpenModule }: { onStartQuiz: () => void; onNavigate: (key: NavKey) => void; onOpenModule: (id: string) => void }) {
  return <div className="page-wrap">
    ...
    <div className="content-grid"><section><div className="section-heading compact"><div><h2>المسارات الموثقة</h2><p>وحدات حقيقية من قاعدة المعرفة، بعدد دروسها الحالي</p></div><button className="text-button" onClick={() => onNavigate("path")}>كل الوحدات <ArrowLeft size={15} /></button></div><div className="module-list">{knowledgeModules.slice(0, 4).map((module, index) => <KnowledgeModuleRow key={module.id} module={module} visual={modules[index % modules.length]} onClick={() => onOpenModule(module.id)} />)}</div></section><aside className="side-column"><div className="quiz-card"><div className="quiz-card-head"><div className="quiz-card-icon"><Brain size={20} /></div><span>تحدي اليوم</span><MoreHorizontal size={18} /></div><h3>هل أنت جاهز لاختبار قصير؟</h3><p>{questions.length} أسئلة أصلية مبنية على مصادر رسمية، بلا أسئلة امتحان مسرّبة.</p><div className="quiz-footer"><span><Clock3 size={14} /> 15 دقيقة</span><button onClick={onStartQuiz}>ابدأ <ArrowLeft size={14} /></button></div></div><div className="source-note"><div className="note-icon"><FileCheck2 size={18} /></div><div><strong>قاعدة المنصة</strong><p>نميز دائمًا بين <b>رسمي</b> و<b>شرح تعليمي</b> و<b>سؤال تدريبي</b>.</p></div></div></aside></div>
...
function KnowledgeModuleRow({ module, visual, onClick }: { module: typeof knowledgeModules[number]; visual: typeof modules[number]; onClick: () => void }) {
  const Icon = visual.icon;
  const sourceLabel = module.sourceIds.map((sourceId) => sourceFor(sourceId)?.publisher ?? sourceId).slice(0, 2).join(" · ");
  return <button className="module-row" onClick={onClick}><div className={`module-icon ${visual.color}`}><Icon size={21} /></div><div className="module-main"><div className="module-title"><span className="module-number">{module.number}</span><strong>{module.title}</strong><SourceStatusBadge status={module.status} /></div><p>{module.description}</p><div className="module-progress"><span className="module-meta"><BookOpen size={13} /> {module.lessonIds.length} دروس <span>·</span> {sourceLabel || "سجل محلي"}</span><span className="module-content-label">{module.status === "access_restricted" ? "يحتاج تحققًا" : "محتوى موثق"}</span></div></div><ChevronLeft size={18} className="row-chevron" /></button>;
}
...
function TrainingPath({ selectedModule, onSelectModule, onStartQuiz }: { selectedModule: string; onSelectModule: (id: string) => void; onStartQuiz: () => void }) {
  ...
  const resolvedModuleId = knowledgeModules.some((module) => module.id === selectedModule) ? selectedModule : (legacyModuleMap[selectedModule] ?? "driver-path");
  const selected = { ...(knowledgeModules.find((module) => module.id === resolvedModuleId) ?? knowledgeModules[0]), lastVerified: knowledgeLessons.find((lesson) => lesson.moduleId === resolvedModuleId)?.lastVerified };
  const selectedVisual = modules[(Number(selected.number) - 1) % modules.length] ?? modules[0];
  const lessons = knowledgeLessons.filter((lesson) => lesson.moduleId === selected.id);
  const evidenceCount = lessons.reduce((total, lesson) => total + lesson.evidenceIds.length, 0);
  const sourceLabel = selected.sourceIds.map((sourceId) => sourceFor(sourceId)?.publisher ?? sourceId).slice(0, 2).join(" · ");
  const Icon = selectedVisual.icon;

  return <div className="page-wrap"><section className="page-heading"><div><div className="eyebrow-line"><BookOpenCheck size={15} /> المنهج الموثق</div><h1>مسار التدريب</h1><p>هذه الشاشة تعرض الدروس الفعلية من قاعدة المعرفة، لا عناوين منفصلة أو وصفًا مكررًا.</p></div><button className="secondary-button" onClick={onStartQuiz}><Brain size={16} /> اختبر نفسك</button></section><div className="path-layout"><aside className="path-nav"><div className="path-nav-head"><span>المنهج الكامل</span><small>{knowledgeModules.length} محورًا</small></div>{knowledgeModules.map((module) => { const ModuleIcon = modules[(Number(module.number) - 1) % modules.length]?.icon ?? BookOpen; return <button key={module.id} onClick={() => { onSelectModule(module.id); setActiveLesson(null); }} className={`path-nav-item ${selected.id === module.id ? "selected" : ""}`}><div className={`module-icon small ${modules[(Number(module.number) - 1) % modules.length]?.color ?? "teal"}`}><ModuleIcon size={16} /></div><div><strong>{module.number} · {module.title}</strong><span>{module.lessonIds.length} دروس · {module.status === "not_verified" || module.status === "access_restricted" ? "يحتاج تحققًا" : "موثق"}</span></div><ChevronLeft size={15} /></button>; })}<div className="path-total"><div className="circular-progress"><BookOpen size={18} /></div><div><strong>المحتوى الكامل</strong><span>{knowledgeLessons.length} درسًا · {knowledgeModules.length} محورًا</span></div></div></aside><section className="lesson-panel"><div className={`lesson-banner ${selectedVisual.color}`}><div className="lesson-banner-copy"><span className="lesson-eyebrow">الوحدة {selected.number} · {selected.status === "official_future" ? "محتوى مستقبلي" : selected.status === "not_verified" || selected.status === "access_restricted" ? "حدود التحقق معلنة" : "محتوى موثق"}</span><h2>{selected.title}</h2><p>{selected.description}</p><SourceBadge>{sourceLabel || "قاعدة المعرفة المحلية"}</SourceBadge></div><div className="lesson-visual"><Icon size={42} /><span>{selected.status === "not_verified" || selected.status === "access_restricted" ? "تحقق" : "مصدر"}</span></div></div><div className="lesson-meta-row"><span><BookOpen size={15} /> {lessons.length} دروس فعلية</span><span><FileCheck2 size={15} /> {evidenceCount} Evidence</span><span><ShieldCheck size={15} /> {selected.lastVerified ?? "سجل محلي"}</span><SourceStatusBadge status={selected.status} /><button className="outline-button" onClick={onStartQuiz}><Play size={14} /> جلسة مراجعة</button></div><div className="lesson-list">{lessons.map((lesson, index) => { const isExpanded = activeLesson === lesson.id; return <article className={`lesson-item ${isExpanded ? "expanded" : ""}`} key={lesson.id}><div className="lesson-item-head"><div className="lesson-check"><span>{String(index + 1).padStart(2, "0")}</span></div><div className="lesson-item-copy"><div className="lesson-item-title-row"><strong>{lesson.title}</strong><SourceStatusBadge status={lesson.status} /></div><p>{lesson.summary}</p><small className="lesson-evidence-count">{lesson.evidenceIds.length > 0 ? `${lesson.evidenceIds.length} Evidence مرتبط بهذا الدرس` : "لا توجد مطالبة تشغيلية مثبتة · راجع حالة المصدر"}</small></div><button className="lesson-open" aria-expanded={isExpanded} onClick={() => setActiveLesson(isExpanded ? null : lesson.id)}>{isExpanded ? "إخفاء التفاصيل" : "فتح الدرس"} <ArrowLeft size={14} /></button></div>{isExpanded && <div className="lesson-detail-expanded knowledge-lesson-detail"><div className="lesson-detail-kicker"><FileCheck2 size={14} /> محتوى الدرس الكامل · من السجل المحلي القابل للمراجعة</div><div className="detail-columns"><div><h4>ما تحتاج إلى معرفته</h4>{lesson.whatYouNeedToKnow.length > 0 ? <ul>{lesson.whatYouNeedToKnow.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="not-verified-note">لا توجد نقاط معرفية مثبتة لهذا الدرس.</p>}</div><div><h4>الإجراء المنشور</h4>{lesson.officialProcedure.length > 0 ? <ol>{lesson.officialProcedure.map((item) => <li key={item}>{item}</li>)}</ol> : <p className="not-verified-note">NOT_VERIFIED — لا يوجد إجراء منشور قابل للعرض.</p>}</div><div><h4>المصطلحات والخطأ الشائع</h4><div className="term-chips">{lesson.terminology.map((term) => <span key={term}>{term}</span>)}</div><p className="mistake-note"><strong>خطأ شائع:</strong> {lesson.commonMistake}</p></div></div>{lesson.scenario && <div className="scenario-card"><div className="scenario-icon"><Target size={17} /></div><div><span>حالة عملية مشتقة من الدليل</span><h4>{lesson.scenario.title}</h4><p><strong>ماذا حدث؟</strong> {lesson.scenario.happened}</p><p><strong>لماذا؟</strong> {lesson.scenario.why}</p><p><strong>ما يثبته المصدر:</strong> {lesson.scenario.officialGuidance}</p><p><strong>الخطوة التالية:</strong> {lesson.scenario.nextStep}</p></div></div>}<div className="evidence-panel"><div className="evidence-panel-head"><div><h4>Evidence panel</h4><p>كل claim هنا مرتبط بالمصدر والصفحة أو القسم.</p></div><span>{lesson.evidenceIds.length} أدلة</span></div>{lesson.evidenceIds.length === 0 && <p className="not-verified-note">{lesson.status === "access_restricted" ? "ACCESS_RESTRICTED — لم يتم الوصول إلى مادة تسجيل الدخول." : "NOT_VERIFIED — لا توجد claims تشغيلية مثبتة لهذا الدرس."}</p>}{lesson.evidenceIds.map((evidenceId) => { const evidence = evidenceFor(evidenceId); if (!evidence) return null; const source = sourceFor(evidence.sourceId); return <div className="evidence-row" key={evidence.id}><div className="evidence-page">{evidence.page}<small>صفحة</small></div><div><strong>{evidence.claim}</strong><span>{evidence.section} · {evidence.topic}</span></div>{source && <a href={source.url} target="_blank" rel="noreferrer" aria-label={`فتح ${source.title}`}><ExternalLink size={14} /></a>}</div>; })}</div><div className="lesson-source-row">{lesson.sourceIds.map((sourceId) => { const source = sourceFor(sourceId); if (!source) return null; return <a key={source.id} href={source.url} target="_blank" rel="noreferrer"><ShieldCheck size={13} /> {source.title} <ExternalLink size={12} /></a>; })}<span>آخر تحقق: {lesson.lastVerified}</span></div></div>}</article>; })}</div><div className="verified-callout"><div><Info size={18} /></div><p><strong>قاعدة التسمية:</strong> الدرس هنا ليس عنوانًا فقط؛ عند الضغط على «فتح الدرس» تظهر النقاط التعليمية والإجراء والمصطلحات والخطأ الشائع والأدلة. وإذا لم توجد مادة رسمية قابلة للقراءة فستظهر <code>NOT_VERIFIED</code> أو <code>ACCESS_RESTRICTED</code> بدل إجابة مخترعة.</p></div></section></div></div>;
}
